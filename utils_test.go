...
func TestGetTxHashFromResponse(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			hash, err := getTxHashFromResponse(tc.out)
			if tc.expError {
...
func TestGetEventsFromTxResponse(t *testing.T) {
	...
	}{
		{
			...
			expEvents: []abcitypes.Event{{
				...
				Attributes: []abcitypes.EventAttribute{
					{Key: "proposal_id", Value: "5", Index: true},
					{Key: "proposal_messages", Value: ",/cosmos.gov.v1.MsgExecLegacyContent", Index: true},
				},
			}},
		},
		{
	...
	for _, tc := range testcases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			events, err := getEventsFromTxResponse(tc.out)
			if tc.expError {
